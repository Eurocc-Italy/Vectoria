...
from vectoria_lib.db_management.preprocessing.extraction_base import ExtractionBase, DocumentData
...
class DocXTextExtractor(ExtractionBase):
    ...
    def extract_text_from_file(self, file_path: Path) -> DocumentData:
        ...
        self.logger.debug("Extracting text from %s", file_path)

        document = docx.Document(file_path)

        document_flat_structure = DocXTextExtractor._extract_content_with_tables(document)
        
        # self.print_document_structure(document_structure)
        
        structured_data, unstructured_data = DocXTextExtractor._structure_data(document_flat_structure) 
                
        return DocumentData(structured_data, unstructured_data)
    


    @staticmethod
    def _extract_content_with_tables(doc):
        ...
        for element in doc.element.body:
            if element.tag.endswith('p'):  # It's a paragraph
                ...
                heading_level = DocXTextExtractor._get_heading_level(paragraph)
                if heading_level:
                ...
                table_data = DocXTextExtractor._extract_table_data(element, doc)
                if not DocXTextExtractor._check_table_empty(table_data):
    ...
    @staticmethod
    def _structure_data(document_flat_structure):
        """
        ...
        [{'childs': [{'childs': [],
                    'name': 'Scopo',
                    'text': ['Lo scopo del presente documento è la descrizione delle '
                            'regole per la gestione, l’elaborazione ed emissione '
                            'dei documenti del sistema normativo della Divisione '
                            'Elettronica.',
                            'Il BMS è lo strumento per la pubblicazione, la '
                    ...
                    {'childs': [],
                    'name': 'Applicabilità',
                    'text': ['Il presente documento si applica alla Divisione '
                            'Elettronica (perimetro Italia).']}],
        'name': 'INTRODUZIONE',
        'text': []},
        {'childs': [{'childs': [],
                    'name': 'Documenti',
                    'text': ['I documenti sotto riportati sono da intendere '
                            'nell’ultima versione correntemente emessa, se non '
                            'diversamente riportato.',
                            "[['Codice', 'Titolo'], ['AQAP-2110', 'NATO Quality "
                            ...
                            "AQAP2310']]",
                            'NOTA: I documenti indicati con (*) sono in fase di '
        ...
        for item in document_flat_structure:
            if not item[0].startswith('Heading'):
                unstructured_data.append(item[1])
            else:
                break
        ...
        for item in document_flat_structure:
            ...
            # If the item is a paragraph or table, add it to the current section
                ...
                if heading_stack:
                    heading_stack[-1]["text"].append(item[1])  # Add paragraph to the current heading
            
            elif item[0] == 'Table':
                table_text = str(item[1])  # Convert the table into string format
                if heading_stack:
                    heading_stack[-1]["text"].append(table_text)  # Add table to the current heading
