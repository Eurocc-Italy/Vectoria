...
def test_qa_agent_without_retriever_with_reranker(config, index_test_folder, clear_inference_engine_cache):
    ...
    context = [
        Document("The goal of reducing sequential computation also forms the foundation of the Extended Neural GPU", metadata={"doc_file_name": "test_docx.docx"}),
        Document("The keys and values are also packed together into matrices K and V . We compute the matrix of outputs as: Attention(Q, K, V ) = softmax(QKT√dk)V (1) The two most commonly used attention functions are additive attention [2], and dot-product (multiplicative) attention. Dot-product attention is identical to our algorithm, except for the scaling factor of √1/dk.", metadata={"doc_file_name": "test_docx.docx"}),
        Document("An attention function can be described as mapping a query and a set of key-value pairs to an output, where the query, keys, values, and output are all vectors. The output is computed as a weighted sum of the values, where the weight assigned to each value is computed by a compatibility function of the query with the corresponding key.", metadata={"doc_file_name": "test_docx.docx"})
    ]
...
def test_qa_agent_without_retriever_with_reranker_with_full_paragraphs(config, index_test_folder, clear_inference_engine_cache):
    ...
    context = [
        Document("The goal of reducing sequential computation also forms the foundation of the Extended Neural GPU", metadata={"doc_file_name": "test_docx.docx", "paragraph_number": "3", "paragraph_name": "Background"}),
        Document("The keys and values are also packed together into matrices K and V . We compute the matrix of outputs as: Attention(Q, K, V ) = softmax(QKT√dk)V (1) The two most commonly used attention functions are additive attention [2], and dot-product (multiplicative) attention. Dot-product attention is identical to our algorithm, except for the scaling factor of √1/dk.", metadata={"doc_file_name": "test_docx.docx", "paragraph_number": "4.2.1", "paragraph_name": "Scaled Dot-Product Attention"}),
        Document("An attention function can be described as mapping a query and a set of key-value pairs to an output, where the query, keys, values, and output are all vectors. The output is computed as a weighted sum of the values, where the weight assigned to each value is computed by a compatibility function of the query with the corresponding key.", metadata={"doc_file_name": "test_docx.docx", "paragraph_number": "4.2", "paragraph_name": "Attention"})
    ]
